class ProductItem extends HTMLElement {
  ...
  onShowClick (handler) {
    this._onShowClick = handler
    this.render()
  }

  onEditClick (handler) {
    this._onEditClick = handler
    this.render()
  }
  ...
  _initEvents () {
    this.querySelector('.button-show').addEventListener('click', () => {
      if (this._onShowClick) {
        this._onShowClick(this._product)
      }
    })
    this.querySelector('.button-edit').addEventListener('click', () => {
      if (this._onEditClick) {
        this._onEditClick(this._product)
      }
    })
    this.querySelector('.button-delete').addEventListener('click', () => {
      if (this._onDeleteClick) {
        this._onDeleteClick(this._product)
      }
    })
  }
        ...
        <div class="group text-left rounded-lg border border-gray-200">
          ...
          <div class="p-3">
            <div class="w-full text-center">
              <button type="button" class="button-show rounded-full w-10 h-10 p-2 bg-indigo-600 hover:bg-indigo-700 text-white">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                </svg>
              </button>
              <button type="button" class="button-edit rounded-full w-10 h-10 p-2 bg-indigo-600 hover:bg-indigo-700 text-white">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                </svg>
              </button>
              <button type="button" class="button-delete rounded-full w-10 h-10 p-2 bg-red-600 hover:bg-red-700 text-white">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
